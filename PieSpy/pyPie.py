...
class Graph:
    ...
    def doLayout(self, iterations):
        visibleNodes = self.getConnectedNodes()
    ...
    def drawImage(self, width, height, borderSize, nodeRadius, edgeThreshold, showEdges):
        ...
        d.text([borderSize, height + borderSize * 2 - 5 - 50 - 18], self.caption, font=ifont, fill=self.config['titleColor'])
        
        ifont = ImageFont.truetype(os.path.join(fontDir,"DejaVuSans.ttf"), 12)
        d.text([borderSize, height+borderSize*2-5-46], "Generated by %s on %s using %s" % (self.config['nick'], self.config['server'], PieInstance.VERSION), font=ifont, fill=self.config['titleColor'])
        d.text([borderSize, height+borderSize*2-5-31], "Blue edge thickness and shortness represents strength of relationship", font=ifont, fill=self.config['titleColor'])
        d.text([borderSize, height+borderSize*2-5-16], "This frame was drawn at %s" % datetime.now(), font=ifont, fill=self.config['titleColor'])
        ...
        for edge in self.edges.values():
            ...
            nodeA = edge.source
            nodeB = edge.target
            x1 = int((width * (nodeA.x - self.minX) / (self.maxX - self.minX)) + borderSize)
            y1 = int((height * (nodeA.y - self.minY) / (self.maxY - self.minY)) + borderSize)
            x2 = int((width * (nodeB.x - self.minX) / (self.maxX - self.minX)) + borderSize)
            y2 = int((height * (nodeB.y - self.minY) / (self.maxY - self.minY)) + borderSize)
        ...
        for node in nodes:
            x1 = int(width * (node.x - self.minX) / (self.maxX - self.minX)) + borderSize
            y1 = int(height * (node.y - self.minY) / (self.maxY - self.minY)) + borderSize
